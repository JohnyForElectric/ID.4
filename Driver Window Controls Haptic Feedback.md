...
<p>
...then 3D printed a "holder" from flexible filament (the red brick on the bottom) for the vibration motor. So, it was easier to attach it and provide soft material around the vibration motor to mimic the way it sits in the steering wheel button assembly. Protected all with beloved Tesa tape for a "professional" look :wink:
...
Future enhancements: (a) add a zener diode to stabilize the voltage on the motor for consistent experience (the voltage does fluctuate a bit), (b) add a small timer circuit without need of power supply to vibrate once briefly after "REAR" LED is turned on, and (c) graduate for option two in the above table... so we can finally start writing code...
